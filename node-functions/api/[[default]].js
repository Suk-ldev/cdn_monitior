...
app.get('/api/zones', async (req, res) => {
  try {
    const platform = req.query.platform || 'cloudflare';

    if (platform === 'edgeone') {
      const { secretId, secretKey } = getEdgeOneKeys();
      
      if (!secretId || !secretKey) {
        return res.status(500).json({ error: "Missing EdgeOne credentials" });
      }
...
app.get('/api/analytics', async (req, res) => {
  try {
    ...
    if (!token || zones.length === 0) {
      return res.status(500).json({ error: 'Missing CF_TOKENS or CF_ZONES configuration' });
    }
...
app.get('/api/traffic', async (req, res) => {
  try {
    ...
    if (platform === 'edgeone') {
      ...
      if (!secretId || !secretKey) {
        return res.status(500).json({ error: "Missing EO_SECRET_ID or EO_SECRET_KEY configuration" });
      }
